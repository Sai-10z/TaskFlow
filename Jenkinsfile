            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: "${SSH_KEY_ID}", keyFileVariable: 'SSH_KEY')]) {
                        def remoteScript = "cd /home/${EC2_USER}/TaskFlow && ([ -d .git ] || (git init && git remote add origin https://github.com/Sai-10z/TaskFlow.git)) && git fetch origin main && git checkout -f main && docker-compose pull && docker-compose down && docker-compose up -d --remove-orphans"
                        if (isUnix()) {
